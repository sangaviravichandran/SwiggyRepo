...
public class SearchInvestigationScenarios extends Setup {
	...
	public void SearchDataInSearchInvestigation(Map<String, String> testdata) throws Exception {
		...
		case NF_DATUM:
			...
			ls.NFdateRangeTo.sendKeys(testdata.get(NF_DATUM_BIS));
			commonUtils.waitTime(1000);
			daoTest.searchClick();
			commonUtils.waitTime(1000);
			daoTest.SearchResult(testdata.get(NF_DATUM_VON),testdata.get(NF_DATUM_BIS), testdata.get(HEADERS));
		...
		case NFID_V_B:
			...
			daoTest.searchClick();
			daoTest.SearchResult(commonUtils.splitString(testdata.get(NF_ID_VON)), commonUtils.splitString(testdata.get(NF_ID_BIS)), testdata.get(HEADERS));
			break;
		case POSTKONTO_NR_AUFTRAGGEBER:
			...
			commonUtils.waitTime(2000);
			commonUtils.type(ls.MoneyReceiver, "enter iban number in money receiver", testdata.get(POSTKONTO_NR_AUFTRAGGEBER));
			commonUtils.waitTime(2000);
			commonUtils.click(ls.NFdateRangeFrom);
			commonUtils.waitTime(2000);
	...
	public void SearchInvestigationDownload(Map<String, String> testdata) throws Exception {
		...
		WebElement AmountRangeTo = ls.AmountRangeTo;
		AmountRangeTo.click();
		commonUtils.type(ls.AmountRangeTo,testdata.get(HEADER_NAME),testdata.get(AMOUNT_TO));
		commonUtils.scrolltoPageHeight();
		daoTest.searchClick();
